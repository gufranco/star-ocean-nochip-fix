...
def an_image(size=0x100000, chipset=0x45, declared=0x0D, at=0x7FC0):
    rom = bytearray(size)
...
class CorrectTest(unittest.TestCase):
    ...
    def test_an_image_needing_nothing_comes_back_unchanged(self):
        already = fix.correct(an_image(chipset=0x00, declared=0x0A))

        self.assertEqual(fix.correct(already), already)

    def test_correcting_twice_changes_nothing_the_second_time(self):
...
class ConfirmTest(unittest.TestCase):
    def test_an_image_the_manifest_knows_is_accepted(self):
        before = an_image()
        edition = an_edition(before, fix.correct(before))

        self.assertIs(fix.confirm(before, edition), edition)

    def test_an_image_of_the_wrong_length_is_refused_before_it_is_hashed(self):
        before = an_image()
    ...
    def test_an_image_of_the_right_length_and_wrong_content_is_refused(self):
        before = an_image()
    ...
    def test_a_manifest_that_disagrees_with_itself_is_reported_as_that(self):
        before = an_image()
        edition = an_edition(before, fix.correct(before))
...
class ApplyTest(unittest.TestCase):
    ...
    def test_the_corrected_image_comes_back(self):
        self.assertEqual(fix.apply(self.before, self.edition), self.after)

    def test_a_result_the_manifest_did_not_predict_is_refused(self):
    ...
    def test_the_refusal_names_what_was_produced_so_it_can_be_looked_up(self):
        wrong = an_edition(self.before, an_image(declared=0x0C))
    ...
    def test_an_image_that_is_not_the_one_named_never_reaches_the_correction(self):
...
class OnDiskTest(unittest.TestCase):
    ...
    def test_a_corrected_file_is_written_beside_the_one_it_came_from(self):
        written = fix.run(self.edition, self.where, self.where)

        self.assertEqual(written.read_bytes(), fix.correct(self.before))

    def test_the_written_file_carries_the_name_the_manifest_gives_it(self):
        written = fix.run(self.edition, self.where, self.where)

        self.assertEqual(written.name, self.edition.writes)

    def test_a_source_that_is_not_there_is_refused(self):
    ...
    def test_running_twice_writes_the_same_file(self):
...
class MainTest(unittest.TestCase):
    ...
    def test_a_directory_with_neither_image_reports_that_nothing_was_done(self):
        self.assertEqual(fix.main([str(self.where), str(self.where)]), 2)

    def test_a_file_under_the_right_name_but_the_wrong_content_is_refused(self):
    ...
    def test_a_file_with_no_header_at_all_is_refused_rather_than_crashing(self):
        edition = editions.EDITIONS[0]
        (self.where / edition.reads).write_bytes(bytes(edition.size))

        self.assertEqual(fix.main([str(self.where), str(self.where / "out")]), 1)

    def test_one_argument_uses_the_destination_in_this_repository(self):
        self.assertEqual(fix.main([str(self.where)]), 2)

    def test_an_image_it_recognises_is_corrected_and_reported(self):
        before = an_image()
        edition = an_edition(before, fix.correct(before))
    ...
    def test_one_present_and_one_absent_still_corrects_the_one_that_is_here(self):
        before = an_image()

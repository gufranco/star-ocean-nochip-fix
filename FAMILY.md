...
The smallest member, and the one closest to the artifact. It corrects the header
of two rebuilt images, which is twelve bytes each, and almost all of its work is
in refusing to touch anything else.

It consumes another member as a git submodule on the import path, which consumes
a third the same way. What a header is and how its checksum is calculated is
`snes-rom-image`; where a header sits is `snes-mapper`, one level further down.
This carries no `[project]` block and publishes nothing, and the download-zip
link on the repository page produces a checkout that cannot run.

Both images are twelve megabytes and neither is here. Everything published is a
digest, and a digest reconstructs nothing. The manifest records the digests a
correct output has, and beside them the ones a superseded output had, with the
upstream commit that changed the answer and why.
